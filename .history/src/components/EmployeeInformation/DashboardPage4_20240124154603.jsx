...
const DashboardPage4 = () => {
  ...
  useEffect(() => {
    if (detail) {
      setMilitaryExperience(detail?.militaryExperience);
      setIfYesSpecialty(detail?.ifYesSpecialty);
      setDateEntered(detail?.dateEntered);
      setDateDischarged(detail?.dateDischarged);
      setNationalGuard(detail?.nationalGuard);
      setValidLicense(detail?.validLicense);
      setDriverLicenseNumber(detail?.validLicence);
      ...
      setHowLongYouKnow(detail?.howLongYouKnow);
    }
  }, [detail]);
