...
const TherapyNotes1 = () => {
  ...
  const checkbox_handler = (value) => {
    setAttitude((prevAttitude) => {
      return prevAttitude.includes(value)
        ? prevAttitude.filter((item) => item !== value)
        : [...prevAttitude, value];
    });
  };
...
<Form.Check
        type="checkbox"
        id="check-api-yes"
        checked={attitude.includes("Cooperative")}
        onChange={() => handleCheckboxChange("Cooperative")}
      >
        <Form.Check.Input name="checkbox1" type="checkbox" isValid required />
        <Form.Check.Label style={{ marginRight: "15px" }}>Cooperative</Form.Check.Label>
      </Form.Check>
