...
const TherapyNotes1 = () => {
  ...
  function check_handler(value, setState) {
    const checkIfPresent = attitude?.some((i) => i === value);
    if (checkIfPresent) {
      const filtered = attitude?.filter((i) => i !== value);
      setAttitude(filtered);
    } else {
      setAttitude((prev) => [...prev, value]);
    }
  }
          ...
          <Form onSubmit={handleSubmit} className="w-100 text-start">
            ...
            >
              ...
              <div className="checkbox-container colored ">
                <CheckBoxMaker
                  setValue={check_handler}
                  value="Cooperative"
                  label="Cooperative"
                  checked={attitude === "Cooperative"}
                  id={"Cooperative"}
                />
                <Form.Check
                  type={"checkbox"}
                  onChange={() => check_handler("Cooperative")}
                  label={"Cooperative"}
                  id={`Cooperative`}
                />
                <Form.Check
