...
import { DateFormatter, InputMaker, TextareaMaker } from "../../../../../Helper/Makers";
...
useEffect(() => {
    if(detail){
        ...
        console.log(item)
        setInvestigationDetails(item?.investigationDetails)
        setInvestigationRecommendationsAndActions(item?.investigationRecommendationsAndActions)
        setInvestigationFollowUp(item?.investigationFollowUp)   
        setInvestigationCompletedBy(item?.investigationCompletedBy)   
        setInvestigationCompletionDate(item?.investigationCompletionDate)   
        setSavedSignedPartB(item?.savedSignedPartB)   
    }
},[detail])
    ...
    <>
      ...
      <div>
        <div className="top-div-personal">
          <Form onSubmit={detail?.data?. submitHandler} className="w-100 text-start">
            ...
            <InputMaker
              ...
              placeholder=""
              type={"date"}
              value={DateFormatter(investigationCompletionDate)}
            />
