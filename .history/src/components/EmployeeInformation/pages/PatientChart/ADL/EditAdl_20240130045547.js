...
const EditAdl = () => {
  ...
  useEffect(() => {
    if (res) {
      ...
      setRequiresNoAssistanceDressing(item?.laundry?.requiresNoAssistance);
      setSomeAssistanceNeededDressing(item?.laundry?.someAssistanceNeeded);
      setCompleteAssistanceNeededDressing(
        item?.laundry?.completeAssistanceNeeded
      );
      setNotApplicableDressing(item?.laundry?.notApplicable);
      setRefusedDressing(item?.laundry?.refused);
      setStaffInitialsDressing(item?.laundry?.staffInitials);
    }
  }, [res]);
