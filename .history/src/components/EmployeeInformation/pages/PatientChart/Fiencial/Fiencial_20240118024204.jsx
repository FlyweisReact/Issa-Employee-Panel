...
import { deleteData, getData, getDataById, postData } from "../../../../api/api";
...
const [patientId, setPatientId] = React.useState("");
  ...
  const patientData = (id) => {
    ...
    axios.get(`${Baseurl}employee/getAllFinancialTransactionsRecord`, {
      headers: {
  ...
  const deleteDataHandler = (id) => {
    ...
    deleteData("employee/employee/deleteFinancialTransactionsRecord", id, patientData(patientId))
    ...
    <>
      ...
      <div>
        ...
        <Form.Group className="mb-3 ">
            <Form.Label style={{ fontWeight: "bold", fontSize: ".9rem",maxWidth:"20%"}}>
              Resident Name:
            </Form.Label>
            <Form.Select onChange={(e) => patientData(e.target.value)} aria-label="Default select example">
  ...
  item?.transactions?.map((transactionItem) => (
    <tr key={transactionItem.id}>
      ...
      <td>{transactionItem.date?.split("T")[0].split("-").reverse().join("-")}</td>
