...
export const I9Form = () => {
  ...
  const [otherLastNames, setOtherLastNames] = useState("PreviousLastName");
  const [address, setAddress] = useState("123 Main St");
  const [aptNumber, setAptNumber] = useState("Apt 456");
  const [city, setCity] = useState("Anytown");
  const [state, setState] = useState("CA");
  const [zipCode, setZipCode] = useState("12345");
  const [dateOfBirth, setDateOfBirth] = useState("1990-01-01");
  const [socialSecurityNumber, setSocialSecurityNumber] = useState("123-45-6789");
  const [email, setEmail] = useState("john.doe@example.com");
  const [telephoneNumber, setTelephoneNumber] = useState("555-555-5555");
  const [citizenshipStatus, setCitizenshipStatus] = useState("U.S. Citizen");
  const [lawfulPermanentResidentText, setLawfulPermanentResidentText] = useState("Some text");
  const [noncitizenAuthorizedToWorkExDate, setNoncitizenAuthorizedToWorkExDate] = useState("2023-12-31");
