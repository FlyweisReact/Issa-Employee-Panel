...
export const I9Form = () => {
  ...
  const [documentTitleC, setDocumentTitleC] = useState("");
  const [issuingAuthorityC, setIssuingAuthorityC] = useState("");
  const [documentNumberC, setDocumentNumberC] = useState("");
  const [expirationDateC, setExpirationDateC] = useState("");
  const [additionInfo, setAdditionalInfo] = useState("");
  const [alternativeProcedureUsed, setAlternativeProcedureUsed] =
    useState(false);

  const [verified, setVerified] = useState(false);
  const [firstDayOfEmployment, setFirstDayOfEmployment] = useState("");
  ...
  const pushInListC = async () => {
    await setListC((prev) => [...prev, listCPayload]);
    setDocumentTitleC("");
    setIssuingAuthorityC("");
    setDocumentNumberC("");
    setExpirationDateC("");
  };
  const popFromListC = (index) => {
    setListC((prev) => prev.filter((_, i) => i !== index));
  };
  // ----
        ...
        <form className="w-100 text-start" onSubmit={submitHandler}>
          ...
          <div className="grid-container">
            ...
            <div className="grid-item">
              <label>Document Title</label>{" "}
              <BorderlessInput
                setState={setDocumentTitleC}
                placeholder={""}
                type={"text"}
                value={documentTitleC}
              />
            </div>
            <div className="grid-item">
              <label>Issuing Authority</label>{" "}
              <BorderlessInput
                setState={setIssuingAuthorityC}
                placeholder={""}
                type={"text"}
                value={issuingAuthorityC}
              />
            </div>
            <div className="grid-item">
              <label>Document Number (if any)</label>{" "}
              <BorderlessInput
                setState={setDocumentNumberC}
                placeholder={""}
                type={"number"}
                value={documentNumberC}
              />
            </div>
            <div className="grid-item">
              <label>Expiration Date (if any)</label>{" "}
              <BorderlessInput
                setState={setExpirationDateC}
                placeholder={""}
                type={"date"}
                value={DateFormatter(expirationDateC)}
              />
            </div>
            <div className="grid-item">
              <button
            ...
            <div className="overflow_table">
              <table className="mb-3 color-full">
                ...
                <tbody>
                  {listC?.map((i, index) => (
                    <tr key={index}>
                      <td> {i.documentTitleC} </td>
                      <td> {i.issuingAuthorityC} </td>
                      <td> {i.documentNumberC} </td>
                      <td> {i.expirationDateC?.slice(0, 10)} </td>
                      <td>
                        <i
                          className="fa-solid fa-trash"
                          onClick={() => popFromListC(index)}
                        />{" "}
                      </td>
                    </tr>
                  ))}
          ...
          <div className="grid-container mb-2">
            ...
            <div className="grid-item full-wid-input">
              <CheckBoxMaker
                setValue={setAlternativeProcedureUsed}
                label={
                  "Check here if you used an alternative procedure authorized by DHS to examine documents"
                }
                checked={alternativeProcedureUsed}
                id={"alternativeProcedureUsed"}
                value={!alternativeProcedureUsed}
              />
            </div>
            <div className="grid-item long-input">
              <label>
            ...
            <div className="grid-item long-input">
              ...
              <BorderlessInput
                setState={setFirstDayOfEmployment}
                placeholder={""}
                type={"date"}
                value={DateFormatter(firstDayOfEmployment)}
              />
            </div>
            ...
            <div className="grid-item">
              ...
              <BorderlessInput
                setState={setEmployerDate}
                placeholder={""}
                type={"date"}
                value={DateFormatter(employerDate)}
              />
            </div>

            <div className="grid-item third-wid-input d-block">
              ...
              <div className="custome-cloud-btn mt-2">
                ...
                <div>
                  {employerSignature && (
                    <p>Digitally Sign by {employerSignature} </p>
                  )}
                </div>
              </div>
            </div>
          ...
          <p className="fw-bold text-center">LISTS OF ACCEPTABLE DOCUMENTS</p>
          <p className="fw-bold text-desc text-center">
            All documents containing an expiration date must be unexpired. <br /> *
            Documents extended by the issuing authority are considered
            unexpired. <br /> Employees may present one selection from List A or a  <br />
            combination of one selection from List B and one selection from List
            C. Examples of many of these documents appear in the Handbook for
            Employers (M-274).{" "}
          </p>
        </form>
      </Container>
