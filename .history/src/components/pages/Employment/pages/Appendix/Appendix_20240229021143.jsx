...
export const Appendix = () => {
  ...
  const [date, setDate] = useState("2023-12-28T00:00:00.000Z");
  const [preferredContactInformation, setPreferredContactInformation] = useState("Preferred Contact Info");
  const [positionHiredFor, setPositionHiredFor] = useState("Position Hired For");
  const [startDate, setStartDate] = useState("2023-01-01T00:00:00.000Z");
  const [spentMoreThan30DaysAbroad, setSpentMoreThan30DaysAbroad] = useState("YES");
  const [closeContactWithActiveTB, setCloseContactWithActiveTB] = useState("NO");
  const [symptomsFever, setSymptomsFever] = useState("YES");
  const [symptomsCough, setSymptomsCough] = useState("NO");
  const [symptomsBloodySputum, setSymptomsBloodySputum] = useState("YES");
  const [symptomsUnintendedWeightLoss, setSymptomsUnintendedWeightLoss] = useState("NO");
  const [symptomsNightSweats, setSymptomsNightSweats] = useState("YES");
  const [symptomsUnexplainedFatigue, setSymptomsUnexplainedFatigue] = useState("NO");
  const [diagnosedWithActiveTB, setDiagnosedWithActiveTB] = useState("YES");
  const [diagnosedWithLatentTB, setDiagnosedWithLatentTB] = useState("NO");
  const [tbTreatmentHistoryYear, setTbTreatmentHistoryYear] = useState("2022");
  const [tbTreatmentHistoryMedication, setTbTreatmentHistoryMedication] = useState("Medication Name");
  const [tbTreatmentHistoryDuration, setTbTreatmentHistoryDuration] = useState("3 months");
  const [tbTreatmentHistoryCompletedTreatment, setTbTreatmentHistoryCompletedTreatment] = useState("YES");
  const [weakenedImmuneSystem, setWeakenedImmuneSystem] = useState("NO");
  const [reviewerSignature, setReviewerSignature] = useState("Reviewer's Signature");
  const [reviewDate, setReviewDate] = useState("2023-12-29T00:00:00.000Z");
