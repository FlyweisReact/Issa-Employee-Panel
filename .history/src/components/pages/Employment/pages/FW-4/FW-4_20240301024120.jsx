...
export const FW4Form = () => {
  ...
  useEffect(() => {
    if (data?.data) {
      ...
      setStep1City(item?.step1City);
      setStep1State(item?.step1State);
      setStep1ZipCode(item?.step1ZipCode);
      setStep1SocialSecurityNumber(item?.step1SocialSecurityNumber);
      setStep1IsNameMatched(item?.step1IsNameMatched);
      setStep1FilingStatus(item?.step1FilingStatus);
      setStep2Choose(item?.step2Choose);
      setStep2Ca(item?.step2Ca);
      setStep2Cb(item?.step2Cb);
      setStep3QualifyingChildrenCredit(item?.step3QualifyingChildrenCredit);
      setStep3OtherDependentsCredit(item?.step3OtherDependentsCredit);
      setStep3TotalCredits(item?.step3TotalCredits);
      setStep4OtherIncome(item?.step4OtherIncome);
      setStep4Deductions(item?.step4Deductions);
      setStep4ExtraWithholding(item?.step4ExtraWithholding);
      setStep5EmployeeSignature(item?.step5EmployeeSignature);
      setStep5Date(item?.step5Date);
      setEmployerName(item?.employerName);
      setEmployerAddress(item?.employerAddress);
      setFirstDateOfEmployment(item?.firstDateOfEmployment);
      setEmployerEIN(item?.employerEIN);
      setStep2bLine1(item?.step2bLine1);
      setStep2bLine2a(item?.step2bLine2a);
      setStep2bLine2b(item?.step2bLine2b);
      setStep2bLine2c(item?.step2bLine2c);
      setStep2bLine3(item?.step2bLine3);
      setStep2bLine4(item?.step2bLine4);
      setStep4bLine1(item?.step4bLine1);
      setStep4bLine2(item?.step4bLine2);
      setStep4bLine3(item?.step4bLine3);
      setStep4bLine4(item?.step4bLine4);
      setStep4bLine5(item?.step4bLine5);
    }
  }, [data]);
