...
const UpdateDischarge = () => {
  ...
  const submitHandler = (e) => {
    ...
    editApi(
      setLoading,
      `employee/createDischargeSummary/${id}`,
      initialFormData
    );
  };
  ...
  useEffect(() => {
    if (data) {
      ...
      setDischargePlanReferralAftercarePlan(item?.dischargePlanReferralAftercarePlan);
      setPatientGuardianSignature(item?.patientGuardianSignature);
      setMedicationUponDischarge(item?.patientGuardianSignatureDate);
      setMedicationUponDischarge(item?.medicationUponDischarge);
...
export default HOC(UpdateDischarge);
