...
const UpdateDTF = () => {
  ...
  useEffect(() => {
    if (detail) {
      ...
      // Combing Hair
      setRequiresNoAssistanceBathingOrShowering(item?.combingHair?.requiresNoAssistance);
      setSomeAssistanceNeededBathingOrShowering(item?.combingHair?.someAssistanceNeeded);
      setCompleteAssistanceNeededBathingOrShowering(item?.combingHair?.completeAssistanceNeeded);
      setNotApplicableBathingOrShowering(item?.combingHair?.notApplicable);
      setRefusedBathingOrShowering(item?.combingHair?.refused);
      setStaffInitialsBathingOrShowering(item?.combingHair?.staffInitials);
    }
  }, [detail]);
