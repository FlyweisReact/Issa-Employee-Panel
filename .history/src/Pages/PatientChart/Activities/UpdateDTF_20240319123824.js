...
const UpdateDTF = () => {
  ...
  useEffect(() => {
    if (detail) {
      ...
      // Shampooing hair
      setRequiresNoAssistanceDressing(item?.shampooingHair?.requiresNoAssistance);
      setSomeAssistanceNeededDressing(item?.shampooingHair?.someAssistanceNeeded);
      setCompleteAssistanceNeededDressing(item?.shampooingHair?.completeAssistanceNeeded);
      setNotApplicableDressing(item?.shampooingHair?.notApplicable);
      setRefusedDressing(item?.shampooingHair?.refused);
      setStaffInitialsDressing(item?.shampooingHair?.staffInitials);
