...
const UpdateDTF = () => {
  ...
  useEffect(() => {
    if (detail) {
      ...
      setSomeAssistanceNeededSelectingClothes(item?.bathingOrShowering?.someAssistanceNeeded);
      setCompleteAssistanceNeededSelectingClothes(item?.bathingOrShowering?.completeAssistanceNeeded);
      setNotApplicableSelectingClothes(item?.bathingOrShowering?.notApplicable);
      setRefusedSelectingClothes(item?.bathingOrShowering?.refused);
      setStaffInitialsSelectingClothes(item?.bathingOrShowering?.staffInitials);
    }
  }, [detail]);
