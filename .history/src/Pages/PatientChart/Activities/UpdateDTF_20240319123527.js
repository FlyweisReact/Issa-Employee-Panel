...
const UpdateDTF = () => {
  ...
  useEffect(() => {
    if (detail) {
      ...
      // Applying Lotion
      setRequiresNoAssistanceCombingHair(item?.applyingLotion?.requiresNoAssistance);
      setSomeAssistanceNeededCombingHair(item?.applyingLotion?.someAssistanceNeeded);
      setCompleteAssistanceNeededCombingHair(item?.applyingLotion?.completeAssistanceNeeded);
      setNotApplicableCombingHair(item?.applyingLotion?.notApplicable);
      setRefusedCombingHair(item?.applyingLotion?.refused);
      setStaffInitialsCombingHair(item?.applyingLotion?.staffInitials);
    }
  }, [detail]);
