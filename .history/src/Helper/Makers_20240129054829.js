...
export const SelectMaker = ({ setValue, options, label }) => {
  return (
    <Form.Group className="mb-3">
      <Form.Label style={{ fontWeight: "bold", fontSize: ".9rem" }}>
        {label}
      </Form.Label>
      <Form.Select onChange={(e) => setValue(e.target.value)} required>
...
export const MultiSelect = ({ options, setValue }) => {
  return (
    <Select isMulti options={options} onChange={(e) => setValue(e)} required />
  );
};
